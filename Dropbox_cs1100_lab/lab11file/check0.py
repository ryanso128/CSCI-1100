...
class BallDraw(object):
    def __init__ (self, parent):
        ...
        self.ball=[]
        for num in range(10):
            balls=Ball(random.randint(10,390), random.randint(10,390)\
                       , random.randint(-8,8), random.randint(-8,8),\
                         random.randint(5,10), \
                        random.choice(["blue", "red", "green", "yellow", "magenta", "orange"]))
            self.ball.append(balls)
        print(self.ball) 
        # self.ball_x,self.ball_y = 0,400   # initial location
        # self.ball_radius = 10
        # self.ball_dx,self.ball_dy = 6,-6   # the movement of the ball object
    ...
    def draw_ball(self):
        ...
        for balls in self.ball:
            bounding_box = (balls.x-balls.radius, \
                        balls.y-balls.radius,\
                        balls.x+balls.radius, \
                        balls.y+balls.radius) 
        self.canvas.create_oval(bounding_box, fill=balls.color)
    ...
    def animate(self):
        ...
        while not self.isstopped:
            # Move the ball
            for balls in self.ball:
                balls.check_reverse(self.maxx, self.maxy)
