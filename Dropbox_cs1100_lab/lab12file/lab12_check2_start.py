...
class MyApp(object):
    ...
    def draw(self):
        self.draw_lines(self.width/2, self.height/2, \
                        min(self.width, self.height)/4, 5)
    ...
    ## recursion ends!
    def draw_lines(self, centerx, centery, length, depth):
        if depth == 0:
            return
        ...
        self.draw_lines(centerx-length, centery, length/2, depth-1)
        self.draw_lines(centerx, centery-length, length/2, depth-1)
        self.draw_lines(centerx+length, centery, length/2, depth-1)
        self.draw_lines(centerx, centery+length, length/2, depth-1)
